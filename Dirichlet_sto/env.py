...
class PublicGoodsEnv:
    ...
    def __init__(self, L=32, r=1.4, R_decay=0.10, use_cumulative_planner_reward=True, episode_length=500):
        ...
        """
        self.L = L
        # 确保 r 始终为 Python float，避免 numpy/torch 标量引发的类型问题
    ...
    def get_state(self):
        ...
        # 当前公共池（以各点为中心的小组的P），按理论最大值 5*r 归一化，范围约 [0,1]
    ...
    def step(self, pi_field):
        ...
        # 用理论最大每个体净收益 5*(r-1) 做归一化，避免 r/L 不同时尺度差异太大
        scale = 5.0 * max(self.r - 1.0, 1e-8)
        norm_avg_net = avg_net / scale

        # 根据开关决定是否累加 planner reward（在 A3C/V-trace 中通常关闭累加）
